...
class KickHubGerencialReporter:
    ...
    def run(self):
        ...
        pulse = self.get_mtd_pulse(conn)
        
        # Growth calculations handling zero denominator (0 to X = 100% growth)
        rv_g = ((pulse['mtd_revenue'] - pulse['last_revenue']) / pulse['last_revenue']) * 100 if pulse['last_revenue'] != 0 else (100.0 if pulse['mtd_revenue'] > 0 else 0.0)
        sl_g = ((pulse['mtd_sales'] - pulse['last_sales']) / pulse['last_sales']) * 100 if pulse['last_sales'] != 0 else (100.0 if pulse['mtd_sales'] > 0 else 0.0)
        tk_g = ((pulse['mtd_ticket'] - pulse['last_ticket']) / pulse['last_ticket']) * 100 if pulse['last_ticket'] != 0 else (100.0 if pulse['mtd_ticket'] > 0 else 0.0)
        ct_g = ((pulse['mtd_new_customers'] - pulse['last_new_customers']) / pulse['last_new_customers']) * 100 if pulse['last_new_customers'] != 0 else (100.0 if pulse['mtd_new_customers'] > 0 else 0.0)
