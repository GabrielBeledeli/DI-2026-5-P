...
export default function ProdutosPage() {
  ...
  const getCategoriaBadge = (categoria?: string) => {
    ...
    if (cat.includes('social')) return <Badge variant="default" className="bg-purple-900/30 text-purple-400 border border-purple-900/50">Social</Badge>;
    if (cat.includes('outdoor')) return <Badge variant="warning">Outdoor</Badge>;
    if (cat.includes('kids')) return <Badge variant="default" className="bg-pink-900/30 text-pink-400 border border-pink-900/50">Kids</Badge>;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-neutral-500">Catálogo de sneakers e controle de estoque.</p>
        </div>
      ...
      <Card>
        ...
        <Table headers={['Nome', 'Categoria', 'Preço', 'Estoque', 'Ações']} isLoading={loading}>
          {filteredProdutos.map((produto) => (
            <TableRow key={produto.id}>
              <TableCell className="font-medium text-white">{produto.nome}</TableCell>
              <TableCell>{getCategoriaBadge(produto.categoria?.nome)}</TableCell>
              <TableCell className="text-white font-medium">
                R$ {produto.preco.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
              </TableCell>
              <TableCell>
                <div className="flex items-center gap-2">
                  <span className={produto.estoque < 5 ? 'text-red-500 font-bold' : 'text-neutral-300'}>
                    {produto.estoque}
                  </span>
                  {produto.estoque < 5 && (
                    <Badge variant="error" className="animate-pulse">Estoque baixo</Badge>
                  )}
                </div>
              </TableCell>
              <TableCell>
